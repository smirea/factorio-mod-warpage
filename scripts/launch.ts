...
const saveFile = resolveSaveFile(parsedArgs.saveSelector, runSavesLink, repoRoot);
...
ensureSymlink(runSavesLink, savesRoot);
...
function resolveSavesRoot(environment: NodeJS.ProcessEnv): string {
  if (environment.SAVES_ROOT && environment.SAVES_ROOT.trim().length > 0) {
    return resolve(environment.SAVES_ROOT);
  }
  if (!environment.HOME) {
    fail('HOME is not set. Set SAVES_ROOT explicitly.');
  }
  return join(environment.HOME, 'Library/Application Support/factorio/saves');
}
